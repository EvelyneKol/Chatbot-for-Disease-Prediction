...
biomistral = pipeline("text-generation", model=gpt2_model, tokenizer=gpt2_tokenizer)
...
def generate_disease_description(disease_name):
    ...
    response = biomistral(prompt, max_new_tokens=100, do_sample=True)
...
if user_input:
    ...
    if "bye" in user_input.lower():
        ...
        if intent == 0:
            ...
            response = biomistral(user_input, max_new_tokens=150, do_sample=True)
            bot_response = response[0]["generated_text"].replace(user_input, "").strip()
